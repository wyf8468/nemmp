...
"""
import unittest
from nmmp.utils.retport.HTMLTestRunner import HTMLTestRunner
from nmmp.common.dir_config import *
import datetime
...
runner = HTMLTestRunner(title="web带截图的测试报告",
                        description="页面功能测试!!",
                        stream=open(htmlreport_dir + r"/" + datetime.datetime.now().strftime('%Y-%m-%d') + r"_test_report.html", "wb"),
                        verbosity=2,
                        retry=2,
                        save_last_try=True)
