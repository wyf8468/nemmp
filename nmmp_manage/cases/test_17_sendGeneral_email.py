...
class TestLogin(unittest.TestCase):
    ...
    def test_sendMakeMms_2_success(self):
        ...
        sgmp(self.driver).send_extractClose()
        comm_frame(self.driver).Frame('mainFrame_105')  # 获取iframe
        sgmp(self.driver).send_select()
        self.driver.switch_to.default_content()  # 释放iframe
        comm_frame(self.driver).Frame('mainFrame_goTemplateList')  # 获取iframe
        time.sleep(1)
        sgmp(self.driver).send_checkbox()
        self.driver.switch_to.default_content()  # 释放iframe
        comm_frame(self.driver).Frame('mainFrame_105')  # 获取iframe
        sgmp(self.driver).send__senderName(gemail_datas['name'], gemail_datas['title'])
        time.sleep(1)
        self.driver.switch_to.default_content()  # 释放iframe
        sgmp(self.driver).send_mailAddress(gemail_datas['address'])
        time.sleep(1)
